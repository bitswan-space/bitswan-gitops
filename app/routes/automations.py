...
@router.post("/deploy")
async def deploy_automations(automation_service: AutomationService = Depends(get_automation_service)):
...
async def stop_automation(deployment_id: str, automation_service: AutomationService = Depends(get_automation_service)):
    return automation_service.stop_automation(deployment_id)

@router.post("/{deployment_id}/restart")
async def restart_automation(deployment_id: str, automation_service: AutomationService = Depends(get_automation_service)):
...
async def create_automation(deployment_id: str, file: UploadFile = File(...), automation_service: AutomationService = Depends(get_automation_service)):
